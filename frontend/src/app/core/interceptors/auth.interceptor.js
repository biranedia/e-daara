...
const refreshSubject = new BehaviorSubject(null);
...
function handle401(req, next, auth, storage, router) {
    if (!isRefreshing) {
        isRefreshing = true;
    ...
    return refreshSubject.pipe(filter((t) => t !== null), take(1), switchMap((t) => next(addToken(req, t))));
}
