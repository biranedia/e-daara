...
export class AdminMessagesComponent implements OnInit {
  ...
  protected readonly allMessages = signal<Message[]>([]);
  ...
  ngOnInit(): void {
    this.load();
    this.recipientCtrl.valueChanges.pipe(
      debounceTime(300),
  ...
  send(): void {
    ...
    if (!r || !this.compose.corps.trim()) return;
  ...
  markRead(m: Message): void {
    this.social.markMessageRead(m.id).subscribe({
      next: () => this.allMessages.update(list =>
