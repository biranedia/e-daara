...
import { StatsSnapshot } from '@core/models';
...
export class AdminStatsComponent implements OnInit {
  ...
  protected readonly snap = signal<StatsSnapshot | null>(null);
  protected readonly history = signal<StatsSnapshot[]>([]);
  protected readonly loading = signal(true);
  protected readonly refreshing = signal(false);

  protected readonly snapshotDate = computed(() =>
    this.snap() ? new Date(this.snap()!.snap_date).toLocaleDateString('fr-FR') : '—'
  );

  protected readonly maxHistory = computed(() =>
    Math.max(...this.history().map(s => Math.max(s.total_users, s.total_inscriptions)), 1)
  );
  ...
  private pct(val: number, total: number): number {
    if (!total) return 0;
